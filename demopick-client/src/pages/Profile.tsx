...
export default function Profile() {
  ...
  return (
    <div className="container mx-auto py-8 px-4 sm:px-6 max-w-4xl font-sans space-y-6">
      ...
      {/* TAB 1: BASIC PROFILE */}
      {activeTab === 'profile' && (
        <Card className="shadow-sm border-slate-200 bg-white rounded-3xl">
          <CardHeader className="text-center pb-4">
            <div className="flex justify-center mb-3">
              <div className="flex h-16 w-16 items-center justify-center rounded-full bg-emerald-50 text-emerald-600 border border-emerald-100 shadow-sm">
                <User className="h-8 w-8" />
              </div>
            </div>
            <CardTitle className="text-2xl font-bold text-slate-900 tracking-tight">Hồ Sơ Cá Nhân</CardTitle>
            ...
            <CardContent className="space-y-5">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-xs font-bold text-emerald-700 hover:text-emerald-800 hover:underline flex items-center gap-1 transition-colors cursor-pointer"
                  >
                ...
                <div className="flex items-center gap-2">
                  <div className="relative flex-1">
                    <Input
                      ...
                      disabled
                      className="bg-[#FAF8F5] border-slate-200 text-slate-700 font-medium text-xs h-10 rounded-xl"
                    />
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200">
                      Đã xác thực
                    </span>
                  </div>
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={handleOpenEmailModal}
                    className="h-10 px-3.5 rounded-xl border-slate-200 hover:bg-slate-50 text-xs font-bold text-slate-700 shrink-0"
                  >
                    Thay đổi
                  </Button>
                </div>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  required
                  className="h-10 rounded-xl border-slate-200 text-xs font-medium focus:border-emerald-500"
                />
              </div>
              ...
              {/* SECURITY BADGE */}
              <div className="pt-1">
                <div className="flex items-center gap-2.5 text-xs text-slate-600 bg-[#FAF8F5] p-3.5 rounded-2xl border border-slate-200/80">
      ...
      {activeTab === 'addresses' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-slate-900">Sổ Địa Chỉ Nhận Hàng</h2>
          ...
          <div className="space-y-3">
            {addresses.map((addr) => {
              return (
                <Card
                  key={addr.id}
                  className={`p-4 rounded-2xl border-2 transition-all bg-white space-y-2 ${
                    addr.isDefault ? 'border-emerald-500/80 shadow-sm' : 'border-slate-200'
                  }`}
                >
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      <Button
                        ...
                        className="h-7 px-2 text-slate-500 hover:text-slate-900 rounded-lg text-xs"
                      >
                  ...
                  <div className="text-xs space-y-0.5 text-slate-600">
                    <div className="font-bold text-slate-900 text-xs sm:text-sm">
                      {addr.recipientName} <span className="font-normal text-slate-500">• {addr.phone}</span>
                    </div>
      ...
      <Dialog open={isAddressModalOpen} onOpenChange={setIsAddressModalOpen}>
        <DialogContent className="sm:max-w-4xl max-w-[95vw] w-full sm:rounded-3xl p-6 sm:p-8 border border-slate-200 bg-white shadow-2xl font-sans max-h-[92vh] overflow-y-auto overflow-x-hidden">
          <DialogHeader className="space-y-1">
            <div className="inline-flex items-center gap-1.5 text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-xl text-xs font-bold w-fit">
          ...
          {showMapPickerInModal ? (
            <div className="py-2">
              <MapLocationPicker
                initialLat={addrLat || 21.0285}
                initialLng={addrLng || 105.8542}
            ...
            <form onSubmit={handleSaveAddressSubmit} className="space-y-4 py-2 text-sm">
              {/* Button to open map */}
              <div className="p-4 bg-gradient-to-r from-emerald-500/10 via-emerald-500/5 to-transparent border border-emerald-300/60 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="space-y-1">
                  <span className="font-bold text-emerald-950 text-sm block flex items-center gap-1.5">
                    <Sparkles className="w-4 h-4 text-emerald-600" /> Chọn vị trí trực tiếp qua Bản đồ OpenStreetMap
                  </span>
              ...
              {/* Label type */}
              <div className="space-y-1.5">
                <Label className="font-bold text-slate-800 text-xs sm:text-sm">Loại địa chỉ:</Label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <Label className="font-bold text-slate-800 text-xs sm:text-sm">Tên người nhận *</Label>
                  <Input
                    value={addrRecipient}
                ...
                <div className="space-y-1.5">
                  <Label className="font-bold text-slate-800 text-xs sm:text-sm">Số điện thoại *</Label>
                  <Input
                    value={addrPhone}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <Label className="font-bold text-slate-800 text-xs sm:text-sm">Tỉnh / Thành phố *</Label>
                  <select
                    value={addrCity}
                    onChange={(e) => setAddrCity(e.target.value)}
                    className="w-full h-10 sm:h-11 px-3 rounded-xl border border-slate-200 bg-white text-xs sm:text-sm font-semibold text-slate-800 focus:ring-2 focus:ring-emerald-500"
                  >
                ...
                <div className="space-y-1.5">
                  <Label className="font-bold text-slate-800 text-xs sm:text-sm">Quận / Huyện</Label>
                  <Input
                    value={addrDistrict}
              ...
              <div className="space-y-1.5">
                <Label className="font-bold text-slate-800 text-xs sm:text-sm">Số nhà, tên đường, khu đô thị *</Label>
                <Input
                  value={addrStreet}
              ...
              <div className="flex items-center gap-2.5 pt-1">
                ...
                />
                <Label htmlFor="defCheck" className="text-xs sm:text-sm font-semibold text-slate-700 cursor-pointer">
              ...
              <DialogFooter className="gap-3 pt-4 border-t">
                <Button
                  ...
                  className="rounded-xl text-xs sm:text-sm font-bold h-11 px-5"
                >
                  Hủy Bỏ
                </Button>
                <Button
      ...
      <Dialog open={isEmailModalOpen} onOpenChange={setIsEmailModalOpen}>
        <DialogContent className="max-w-md sm:rounded-3xl p-6 border border-slate-200 bg-white shadow-2xl font-sans">
          ...
          {otpStep === 'input_email' ? (
            <form onSubmit={handleSendOtp} className="space-y-4 pt-2">
              <div className="space-y-2">
                <Label htmlFor="newEmailInput" className="text-xs font-bold text-slate-800">
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-4 pt-2">
              <div className="space-y-2">
                <Label htmlFor="otpInput" className="text-xs font-bold text-slate-800">
