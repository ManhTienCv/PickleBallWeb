...
export default function ProductCard({ product, onAddToCart }: ProductCardProps) {
  ...
  const handleModalAddToCart = () => {
    ...
    cartService.addToCart(variantId, modalQuantity, product).catch(() => {
              ...
              {product.variants && product.variants.length > 0 && (
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-slate-800 dark:text-slate-200">
                    Chọn phiên bản / Thông số:
                  </label>
                  <div className="flex flex-wrap gap-2">
                    {product.variants.map((v) => {
                      const isSelected = selectedVariant?.id === v.id
                      const label = v.option_value || v.color || v.weight || `Bản #${v.id}`
                      return (
                        <button
                          key={v.id}
                          type="button"
                          ...
                          className={`px-3 py-1.5 rounded-xl text-xs font-medium border transition-all flex items-center gap-1.5 ${
                            isSelected
                              ...
                              : 'border-slate-200 dark:border-border hover:border-slate-300 text-slate-700 dark:text-slate-300'
                          }`}
                        >
              ...
              <div className="space-y-3 pt-3 border-t border-slate-100 dark:border-border">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Button
                    ...
                    className="h-10 text-xs font-medium rounded-xl border-slate-300 dark:border-border bg-white dark:bg-card hover:bg-slate-50 dark:hover:bg-slate-800 text-foreground gap-1.5 active:scale-95 transition-all"
                  >
                  ...
                  <Button
                    ...
                    className="h-10 text-xs font-bold rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white shadow-md shadow-emerald-500/20 gap-1.5 active:scale-95 transition-all"
                  >
                    <span>Mua ngay</span>
